# file : k_means.py
# breief : implement for k_means
# author : rbguo 
# history: 20180608
# reference:    https://blog.csdn.net/zuoyonggang123/article/details/79879557
import numpy  
import random  
import codecs  
import copy  
...
def getCentroids(clusterDict):  
    ...
    for key in clusterDict.keys():  
        ...
        #print ("###################　getCentroids ,key =%s ,centroid =%s" %(key, centroid ) )
...
def getVar(clusterDict, centroidList):  
    ...
    sum = 0.0  
    for key in clusterDict.keys():  
        #print('## getVar key = %s,centroidList[key] = %s'%(key,centroidList[key]))
        vec1 = numpy.array(centroidList[key])   ##　rbguo 这里的vec1就是选取的4个点(坐标)中的一个
        distance = 0.0  
        for item in clusterDict[key]:       ## rbguo 计算属于某一簇中的所有点到中心点的距离
            vec2 = numpy.array(item)  
            distance += calcuDistance(vec1, vec2)  
        sum += distance         ## rbguo 竟然是求累计的和
...
if __name__ == '__main__':  
    ...
    #print("###clusterDict = ",clusterDict)      ### rbguo added 得到了每个点属于哪个簇
    
    ##rbguo added 将簇类中各个向量与质心的距离进行累加求和 newVar就是得到的和  注意这里newvar衡量了4个簇，而不是一个。
